...
const maxEssai = 7;
let nbErreur = 0;
let LettreJoues = document.getElementById('lettreJoues');
let divLettreJoues = document.getElementById('divLettreJoues');
let zoneDeJeu = document.querySelector(".container>.row:first-child");
...
const hideDivLettreJoues = ()=>{
    divLettreJoues.innerHTML="";
};

//permet d'afficher l'image et le son de la défaite
const perdu = () => {
    //partie son
    const looseSound = new Audio('assets/sound/BALTRINGUEUX.mp3');
    looseSound.play();
...
function verifierSaisie(lettre, motMystere) {
    ...
    if (nbErreur >= maxEssai) {
        ...
        //on affiche la lettre joué
        LettreJoues.innerText = LettreJoues.innerText + lettre + '-';
